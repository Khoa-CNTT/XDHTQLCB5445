...
const ReportForm = () => {
    ...
    try {
      ...
      bookings.forEach(booking => {
        ...
        if (staffInfo.UserID?.firstName) {
          employeeName = `${staffInfo.UserID.firstName} ${staffInfo.UserID.lastName || ''}`.trim();
  ...
  const getStaffBookingCounts = (staffs) => {
    return staffs.map(staff => ({
      ...
      pending: staff.totalBookings - staff.completedBookings - staff.cancelledBookings
    }));
  };
    ...
    try {
      if (type === 'order') {
        ...
        filtered.forEach(booking => {
          ...
          revenueByDate[dateStr] = (revenueByDate[dateStr] || 0) + (booking.totalAmount || 0);
        });
        ...
        const totalCompleted = staffData.reduce((sum, staff) => sum + staff.completedBookings, 0);
        const totalRevenue = staffData.reduce((sum, staff) => sum + staff.totalRevenue, 0);
  ...
  const staffColumns = [
      ...
      render: (text, record) => (
        <span>
          <Progress
            ...
            size="small"
            status={record.completionRate > 70 ? 'success' : record.completionRate > 50 ? 'normal' : 'exception'}
          />
        </span>
      ),
    ...
    },
    {
      ...
      render: (text, record) => (
        <span>
          {record.averageRating > 0 ? record.averageRating.toFixed(1) + '/5' : 'Chưa có'}
        </span>
      ),
    ...
    <Card title="Báo cáo theo thời gian">
      <Form layout="vertical" form={form} onFinish={handleSearch} initialValues={{ type: 'order' }}>
        <Row gutter={16}>
          ...
          <Col span={6} style={{ display: 'flex', alignItems: 'end', gap: '8px' }}>
...
{/* Phần order */}
          ...
          {/* Phần booking */}
          {form.getFieldValue('type') === 'booking' && revenuePieData.length > 0 && (
            <Card title="Biểu đồ doanh thu theo ngày" style={{ marginTop: 30 }}>
              <ResponsiveContainer width="100%" height={400}>
                <PieChart>
                  <Pie
          ...
          {form.getFieldValue('type') === 'staff' && staffPerformance.length > 0 && (
            <>
              <Card title="Hiệu suất làm việc của nhân viên" style={{ marginTop: 30 }}>
                <Table
              ...
              <Card title="Biểu đồ số lượng lịch hẹn theo nhân viên" style={{ marginTop: 30 }}>
                <ResponsiveContainer width="100%" height={400}>
                  <BarChart
                  ...
                  >
                    ...
                    <Tooltip />
                    <Legend />
                    <Bar dataKey="completed" name="Hoàn thành" fill="#00C49F" />
              ...
              <Card title="Top 5 nhân viên có doanh thu cao nhất" style={{ marginTop: 30 }}>
                <ResponsiveContainer width="100%" height={300}>
                  ...
                  >
                    ...
                    <Tooltip formatter={(value) => `${value.toLocaleString()} ₫`} />
                    <Bar dataKey="totalRevenue" name="Doanh thu" fill="#8884d8" />
                  </BarChart>
                </ResponsiveContainer>
              </Card>
            </>
          ...
          {!loading && reportData.quantity === 0 && reportData.revenue === 0 && <EmptyState />}
        </>
      )}
    </Card>
  );
